  ...
  return (
    <div className={styles.container}>
      ...
      {isValid && (
        <div className={styles.resultSection}>
          ...
          <div className={styles.resultCard}>
            <span className={styles.resultLabel}><LaTeX>{'k_{12}'}</LaTeX> (calculated)</span>
      ...
      <div className={styles.comparisonSection}>
        <h4 className={styles.compTitle}>Compare with Experiment</h4>
        <div className={styles.field}>
          <label><LaTeX>{'k_{12}'}</LaTeX> (experimental, M⁻¹s⁻¹)</label>
        ...
        {isValid && kExpv > 0 && (
          <div className={styles.verdict}>
            <div className={styles.ratioDisplay}>
              Ratio k<sub>exp</sub>/k<sub>calc</sub> = <strong>{ratio.toFixed(2)}</strong>
            </div>
            <div className={`${styles.mechBadge} ${isOuterSphere ? styles.outer : styles.inner}`}>
              {isOuterSphere
                ? '✅ Consistent with Outer-Sphere mechanism'
                : '⚠️ Inconsistent — likely Inner-Sphere mechanism'}
            </div>
          </div>
        )}
      </div>
