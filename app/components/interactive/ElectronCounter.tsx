...
const METALS: Metal[] = [
  { symbol: 'Ti', group: 4, neutralE: 4, ionicE: 4, label: 'Ti (Group 4)' },
  { symbol: 'V', group: 5, neutralE: 5, ionicE: 5, label: 'V (Group 5)' },
  { symbol: 'Cr', group: 6, neutralE: 6, ionicE: 6, label: 'Cr (Group 6)' },
  { symbol: 'Mn', group: 7, neutralE: 7, ionicE: 7, label: 'Mn (Group 7)' },
  { symbol: 'Fe', group: 8, neutralE: 8, ionicE: 8, label: 'Fe (Group 8)' },
  { symbol: 'Co', group: 9, neutralE: 9, ionicE: 9, label: 'Co (Group 9)' },
  { symbol: 'Ni', group: 10, neutralE: 10, ionicE: 10, label: 'Ni (Group 10)' },
  { symbol: 'Mo', group: 6, neutralE: 6, ionicE: 6, label: 'Mo (Group 6)' },
  { symbol: 'Ru', group: 8, neutralE: 8, ionicE: 8, label: 'Ru (Group 8)' },
  { symbol: 'Rh', group: 9, neutralE: 9, ionicE: 9, label: 'Rh (Group 9)' },
  { symbol: 'Pd', group: 10, neutralE: 10, ionicE: 10, label: 'Pd (Group 10)' },
  { symbol: 'Ir', group: 9, neutralE: 9, ionicE: 9, label: 'Ir (Group 9)' },
  { symbol: 'Pt', group: 10, neutralE: 10, ionicE: 10, label: 'Pt (Group 10)' },
];

const LIGANDS: Ligand[] = [
  { name: 'CO (carbonyl)', symbol: 'CO', neutralE: 2, ionicE: 2, charge: 0 },
  { name: 'PPh₃ (phosphine)', symbol: 'PPh₃', neutralE: 2, ionicE: 2, charge: 0 },
  { name: 'η⁵-Cp (cyclopentadienyl)', symbol: 'η⁵-Cp', neutralE: 5, ionicE: 6, charge: -1 },
  { name: 'η⁶-C₆H₆ (benzene)', symbol: 'η⁶-arene', neutralE: 6, ionicE: 6, charge: 0 },
  { name: 'Cl⁻ (chloride)', symbol: 'Cl⁻', neutralE: 1, ionicE: 2, charge: -1 },
  { name: 'H⁻ (hydride)', symbol: 'H⁻', neutralE: 1, ionicE: 2, charge: -1 },
  { name: 'η²-C₂H₄ (ethylene)', symbol: 'η²-C₂H₄', neutralE: 2, ionicE: 2, charge: 0 },
  { name: 'CH₃⁻ (methyl)', symbol: 'CH₃⁻', neutralE: 1, ionicE: 2, charge: -1 },
  { name: 'NO⁺ (nitrosyl)', symbol: 'NO⁺', neutralE: 3, ionicE: 2, charge: 1 },
  { name: 'M-M bond', symbol: 'M-M', neutralE: 1, ionicE: 1, charge: 0 },
];
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.methodToggle}>
        <button className={`${styles.toggleBtn} ${method === 'ionic' ? styles.activeToggle : ''}`} onClick={() => setMethod('ionic')}>Ionic Method</button>
        <button className={`${styles.toggleBtn} ${method === 'covalent' ? styles.activeToggle : ''}`} onClick={() => setMethod('covalent')}>Covalent Method</button>
      </div>

      <div className={styles.ligandDrawer}>
        <p className={styles.drawerLabel}>Add Ligands:</p>
      ...
      {addedLigands.length > 0 && (
        <div className={styles.addedList}>
          <p className={styles.drawerLabel}>Current Ligands:</p>
          {addedLigands.map(a => (
      ...
      <div className={`${styles.result} ${styles[status]}`}>
        <div className={styles.resultBreakdown}>
          ...
          <div>+ Ligands: {addedLigands.reduce((sum, a) => sum + (method === 'ionic' ? a.ligand.ionicE : a.ligand.neutralE) * a.count, 0)}e⁻</div>
          {method === 'covalent' && overallCharge !== 0 && <div>− Charge correction: {overallCharge}e⁻</div>}
        </div>
        ...
        <div className={styles.statusBadge}>
          {status === 'stable' && '✅ 18e⁻ — Stable (closed shell)'}
          {status === 'allowed' && '🟡 16e⁻ — Stable for d⁸ square planar'}
          {status === 'over' && `⚠️ ${totalE}e⁻ — Anti-bonding electrons occupied`}
          {status === 'under' && `🔵 ${totalE}e⁻ — Electron-deficient (reactive)`}
        </div>
      </div>
    </div>
  );
